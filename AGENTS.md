...
- Active line of work: **Phase 2 · SP3b** (solver dimensions, delivered in slices). Work happens on `main` unless the handoff says otherwise.
...
- Verify with `npx vitest run && npx tsc --noEmit && npx eslint scripts src tests`. The handoff records the expected pass count; anything less is a regression.

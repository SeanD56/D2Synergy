...
> ⚠️ **Ingestion finding (Phase 0, 2026-07-22):** the assumptions above **do not match the live Manifest.** `DestinyArtifactDefinition` currently exposes **one** artifact — the active seasonal one (*"Implement of Curiosity"*) — with **5 tiers of 7 perks**, not 7 artifacts of 3×7. Bungie appears to prune past-season artifacts from the table. Phase 0 ingests whatever the Manifest holds (`tiers[] × items[]`, generically), so the derived data is correct; but the **build-model logic** (tier-ceiling, "evaluate all 7 artifacts", 21-perk cap) must be reworked against this reality in a later phase. Revisit the solver treatment before Phase 1 feasibility work.

...
- **Set bonuses: BLOCKED by the VALIDATOR CONTRACT, and now precisely diagnosed.** Two earlier readings in this file were wrong, so here is the measured position:
  - ✅ **A synergy objective DOES exist.** 58 of 112 set bonuses carry keyword tags across 40 of 56 sets (e.g. "Wrecker" produces overshield + stasis_crystal at 2 pieces; "Ascendant Escape" produces invisibility at 4). An earlier note claimed set-bonus effects were untagged — that was checking `tags` at the SET level, but tags live on each BONUS. There is plenty to optimise for.
  - ⛔ **The blocker is `setBonusCounts` in `src/lib/validation/armor.ts`.** It validates every declared `armor.setBonuses` entry against how many pieces of that set appear in `armor.pieces` — and the solver never writes `pieces`. So any bonus the solver prescribed would evaluate as `have = 0 < requiredCount` and emit `SET_COUNT_INVALID` on **every** build.
  - **THE DECISION THAT UNBLOCKS IT is about what `setBonuses` MEANS**, and it is user-facing: today it asserts "these bonuses are ACTIVE" (a claim about equipped pieces). A solver working by prescription needs "these bonuses are TARGETED" (a claim about what to obtain). Options: (a) wait for SP4 to populate `pieces`; (b) add a distinct prescription field so the two claims stay separable; (c) relax `setBonusCounts` to skip validation when `pieces` is empty — cheapest, but it makes the rule silently non-enforcing exactly when a build is solver-generated, which is the same class of mistake as the `pieces` ∪ `exoticHash` gap already recorded above.
  - **Slot arithmetic, for whichever way it goes:** 5 armour slots minus 1 exotic leaves **4** legendary slots, and bonuses require 2 or 4 pieces — so the legal plans are one 4-piece bonus, two different 2-piece bonuses, or a single 2-piece. Also note DIM models — and our own data confirms — a set-bonus **SELECTOR socket** (`selectors`, on 3 exotic class items) letting one piece wildcard a missing set piece; ignoring it would over-report infeasibility.

...
**Phase 2 · SP2 (artifact capacity oracle) — ✅ SHIPPED to `main`** (merge `a7415e4` from `phase2-artifact-capacity`). Whole-branch reviewed (opus): ready-to-merge, no Critical/Important. Baseline at merge: **73/73 tests pass**, `tsc --noEmit` clean, `eslint scripts src tests` clean. NOTE: `main` is ahead of `origin/main` — **not yet pushed**.

**NEXT ACTION:** start SP3 — the completion / beam-search solver. No spec/plan exists yet: invoke `superpowers:brainstorming`, then writing-plans → subagent-driven-development (same flow as SP1/SP2). SP3 consumes the SP2 capacity oracle (see below) and `scoreSynergy` from SP1.

Phase 2 was decomposed into sub-projects (see design doc / SP1 spec). Order + status:
- **SP1 — synergy engine** ✅ shipped.
- **SP2 — artifact capacity oracle** ✅ shipped. Reframed from the original "rework `Build.artifact`" — see Decisions → Artifacts (SP2). `Build.artifact` stays a flat `selectedPerkHashes` list; SP2 delivered a **pure feasibility oracle** instead of a data-model rework.
- **SP3 — completion / beam-search solver** ⏭️ NEXT (needs SP1 + SP2). Ranks completions by stat-fit + `scoreSynergy`; uses the SP2 oracle for artifact-perk legality/pruning.
...
- **SP3 — completion / beam-search solver (NEXT):** decompose + inverted indexes + beam search; ranks completions by stat-fit + `scoreSynergy` (SP1). Uses the SP2 oracle for artifact-perk feasibility during search. No spec/plan yet.
